...
def infer_area_type(location):
    ...
    cbd_keywords = ["new york","manhattan","brooklyn","queens","bronx","staten island",
        ...
        "toronto","montreal","vancouver","ottawa","calgary","edmonton","winnipeg",
        "halifax","victoria","quebec city","mexico city","guadalajara","monterrey","tijuana"]
    suburb_keywords = ["westmount","laval","longueuil","brossard","côte-saint-luc","ndg",
        "saint-laurent","west island","mississauga","brampton","markham","vaughan",
        "richmond hill","pickering","ajax","oshawa","milton","oakville","burlington",
        "burnaby","surrey","richmond bc","coquitlam","delta","langley","maple ridge",
        "north vancouver","west vancouver","okotoks","airdrie","sherwood park",
        "st. albert","gatineau","kanata","orleans","cambridge","brookline","somerville",
        "newton","quincy","jersey city","hoboken","newark","yonkers","staten island",
        "flushing","long island city","bronxville","white plains","oakland","berkeley",
        "san mateo","redwood city","palo alto","pasadena","burbank","santa monica",
        "long beach","anaheim","evanston","oak park","naperville","schaumburg",
        "coral gables","hialeah","kendall","aventura","zapopan","tlajomulco",
        "santa catarina","san nicolas de los garza"]
...
def filter_duplicates(df):
    preferred_statuses = {"Under Construction","Contract Signed","IC Approved",
    ...
    try:
        with st.spinner("Loading, please wait..."):
            ...
            if response.status_code != 200:
                ...
                # --- Radius Legend INSIDE MAP (Top-Right) ---
                legend_template = f"""
                    {{% macro html(this, kwargs) %}}
                    <div style='position: absolute; top: 10px; right: 10px; width: 150px; z-index: 9999;
...
Do you want me to also add the PowerPoint export section back at the end (I stopped here due to space) or keep it exactly like above but complete with export code?
...
ChatGPT said:
Here is your entire full script including:

Zoom in/out control (top-left, default position)

Radius legend inside the map (top-right)
            ...
            if response.status_code != 200:
                ...
                if st.button("\U0001F4E4 Export to PowerPoint"):
                    try:
                        ...
                        add_centres_to_slide_table(closest.to_dict("records"), "Details of 5 Closest Centres")
                        output_pptx = os.path.join(tempfile.gettempdir(), "closest_centres.pptx")
                        prs.save(output_pptx)
                        with open(output_pptx, "rb") as file:
                            st.download_button("⬇ Download PowerPoint", file, file_name="closest_centres.pptx")
