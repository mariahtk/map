...
# --- JavaScript to remove "Manage App" or other floating elements ---
components.html("""
<script>
const killFloaters = () => {
...
if input_address:
    try:
        ...
        if response.status_code != 200:
            ...
            all_data = [pd.read_excel(file_path, sheet_name=s, engine="openpyxl").assign(**{"Source Sheet": s}) for s in sheets]
            ...
            # --- Comps takes priority: Remove rows from other sheets if Centre Number exists in Comps ---
            comps_numbers = combined_data.loc[combined_data["Source Sheet"] == "Comps", "Centre Number"].unique()
            ...
            # --- Deduplicate with sheet priority (Active Centre=0, Comps=1, Centre Opened=2) ---
            ...
            # --- Transaction Milestone Status override from Active Centre ---
            ...
            active_status_map = active_df.dropna(subset=["Centre Number", "Transaction Milestone Status"]).set_index("Centre Number")["Transaction Milestone Status"].to_dict()
            ...
            for _, row in closest.iterrows():
                ...
                popup_text = (f"#{int(row['Centre Number'])} - {row['Addresses']} | "
            ...
            uploaded_image = st.file_uploader("\U0001F5BC\ufe0f Optional: Upload Map Screenshot for PowerPoint",
                                              type=["png", "jpg", "jpeg"])
            if st.button("\U0001F4E4 Export to PowerPoint"):
                try:
                    ...
                    rows, cols = len(closest) + 1, 6
                    table = slide.shapes.add_table(rows, cols, Inches(0.5), top_text, Inches(9), Inches(0.8 + 0.4 * rows)).table
                    ...
                    for i, row in enumerate(closest.itertuples(), start=1):
                        ...
                        table.cell(i, 1).text = row._asdict().get("Addresses", "")
                        city = row._asdict().get("City", "")
                        state = row._asdict().get("State", "")
                        zipcode = row._asdict().get("Zipcode", "")
                        table.cell(i, 2).text = f"{city}, {state} {zipcode}"
                        table.cell(i, 3).text = row._asdict().get("Format - Type of Centre", "")
                        table.cell(i, 4).text = row._asdict().get("Transaction Milestone Status", "")
                        dist_val = row._asdict().get("Distance (miles)", 0)
                        table.cell(i, 5).text = f"{dist_val:.2f}"

                    pptx_path = f"Closest_Centres_{input_address.replace(' ', '_')}.pptx"
                    prs.save(pptx_path)
                    with open(pptx_path, "rb") as f:
                        st.download_button("Download PowerPoint", f.read(), file_name=pptx_path)
                except Exception:
                    st.error(f"Error exporting PowerPoint: {traceback.format_exc()}")
