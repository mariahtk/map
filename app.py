...
def infer_area_type(location):
    ...
    cbd_keywords = ["new york","manhattan","brooklyn","queens","bronx","staten island",
        ...
        "houston","seattle","miami","atlanta","dallas","phoenix","detroit","san diego",
        "minneapolis","denver","austin","portland","nashville","new orleans","las vegas",
        "toronto","montreal","vancouver","ottawa","calgary","edmonton","winnipeg","halifax",
        "victoria","quebec city","mexico city","guadalajara","monterrey","tijuana"]
    suburb_keywords = ["westmount","laval","longueuil","brossard","côte-saint-luc","ndg",
        "saint-laurent","west island","mississauga","brampton","markham","vaughan","richmond hill",
        "pickering","ajax","oshawa","milton","oakville","burlington","burnaby","surrey","richmond bc",
        "coquitlam","delta","langley","maple ridge","north vancouver","west vancouver","okotoks",
        "airdrie","sherwood park","st. albert","gatineau","kanata","orleans","cambridge","brookline",
        "somerville","newton","quincy","jersey city","hoboken","newark","yonkers","staten island",
        "flushing","long island city","bronxville","white plains","oakland","berkeley","san mateo",
        "redwood city","palo alto","pasadena","burbank","santa monica","long beach","anaheim","evanston",
        "oak park","naperville","schaumburg","coral gables","hialeah","kendall","aventura","zapopan",
        "tlajomulco","santa catarina","san nicolas de los garza"]
...
def normalize_centre_number(val):
    return str(val).strip().lstrip("0") or "0" if pd.notna(val) else ""

def normalize_address(addr):
    return addr.strip().lower() if pd.notna(addr) else ""

def filter_duplicates(df):
    preferred_statuses = {"Under Construction","Contract Signed","IC Approved","Not Paid But Contract Signed","Centre Open"}
    ...
    return grouped.apply(select_preferred).reset_index(drop=True).drop(columns=["Normalized Address"])
        ...
        with st.spinner("Loading, please wait..."):
            ...
            if response.status_code != 200 or not data.get('results'):
                ...
                for col in ["City","State","Zipcode"]:
                    if col not in combined_data.columns: combined_data[col] = ""
                combined_data["Distance (miles)"] = combined_data.apply(lambda row: geodesic(input_coords,(row["Latitude"],row["Longitude"])).miles, axis=1)
                closest = combined_data.sort_values("Distance (miles)").head(5).copy()

                # --- Map with radius and markers ---
                m = folium.Map(location=input_coords, zoom_start=14, zoom_control=False, control_scale=True)
                folium.Marker(location=input_coords, popup=f"Your Address: {input_address}", icon=folium.Icon(color="green")).add_to(m)
                for _, row in closest.iterrows():
                    folium.PolyLine([input_coords,(row["Latitude"],row["Longitude"])], color="blue", weight=2.5).add_to(m)
                    folium.Marker(location=(row["Latitude"],row["Longitude"]),
                                  popup=f"#{int(row['Centre Number'])} - {row['Addresses']}", 
                                  tooltip=row["Addresses"], 
                ...
                # --- Legend + Zoom Toggle JS ---
                legend_zoom_template = """
                {% macro html(this, kwargs) %}
                <div style='position: absolute; bottom: 50px; left: 10px; width: 160px; z-index: 9999;
                            background-color: white; padding: 10px; border: 2px solid gray;
                            border-radius: 5px; font-size: 14px;'>
                    <b>Radius</b><br>
                    <span style='color:green;'>&#x25CF;</span> {{radius_miles}}-mile Zone
                    <br><br>
                ...
                <script>
                    ...
                    setTimeout(() => {
                        if (!map) { map = document.querySelector('.leaflet-container')._leaflet_map; }
                    }, 500);
                </script>
                {% endmacro %}
                """.replace("{{radius_miles}}", str(radius_miles.get(area_type,5)))
                legend_zoom = MacroElement()
                legend_zoom._template = Template(legend_zoom_template)
                m.get_root().add_child(legend_zoom)
                ...
                with col2:
                    st.write("Legend + Export options")
