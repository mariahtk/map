...
if input_address:
    try:
        ...
        if response.status_code != 200:
            ...
            with col2:
                st.markdown(""" 
                    <div style="background-color: white; padding: 10px; border: 2px solid grey; border-radius: 10px; width: 100%; margin-top: 20px;">
                        ...
                        <hr>
                        <i style="color: green; font-size: 18px;">&#9679;</i> 5 Mile Radius
                    </div>
            ...
            if not group1.empty:
                add_distance_slide(prs, "Closest Centres (1-4)", group1)
            if not group2.empty:
                add_distance_slide(prs, "Closest Centres (5)", group2)

            pptx_path = "Closest_Centres_Presentation.pptx"
            prs.save(pptx_path)
            st.success(f"PowerPoint presentation saved as {pptx_path}")
